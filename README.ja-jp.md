...
- **14 タブ構成**:
  ...
  - **Events** — ソース単位にグルーピングされたイベント一覧。フィルタ（`type:` / `id:` / `is:active|error` / `sort:` / `has:<text>`（全ソース横断のパラメータ検索）、列見出しクリックでも並び替え可能）とアプリ内構文ヘルプ、常時表示のエラー件数、キーボード操作、イベントトレース詳細（`t=` / `st=` / `[dt=]`）に対応。HTTP リクエストを選択すると Request/Response サマリー（メソッド・URL・ステータス・ヘッダー・所要時間）が表示されます。

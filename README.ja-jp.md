...
- **14 タブ構成**:
  ...
  - **Events** — ソース単位にグルーピングされたイベント一覧。フィルタ（`type:` / `id:` / `is:active|error` / `sort:`、列見出しクリックでも並び替え可能）とアプリ内構文ヘルプ、常時表示のエラー件数、キーボード操作、イベントトレース詳細（`t=` / `st=` / `[dt=]`）に対応。HTTP リクエストを選択すると Request/Response サマリー（メソッド・URL・ステータス・ヘッダー・所要時間）が表示されます。
  - **Timeline** — ソケット数・リクエスト数・転送バイト数などの時系列をキャンバスに描画。マウスのほかキーボード（矢印キー/+-/Home）でもズーム・パンでき、ホバーで値を確認でき、現在のズーム倍率と表示中の時間帯も常時表示されます。

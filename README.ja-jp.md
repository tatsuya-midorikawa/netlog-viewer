...
- **14 タブ構成**:
  - **Import** — エクスポート日時・キャプチャモード・ブラウザー/OS 情報・コマンドライン・ユーザーコメント。コマンドラインとコメントにはコピーボタンを製備。
  - **Events** — ソース単位にグルーピングされたイベント一覧。フィルタ（`type:` / `id:` / `is:active|error` / `sort:` / `has:<text>`（全ソース横断のパラメータ検索）、列見出しクリックでも並び替え可能）とアプリ内構文ヘルプ、常時表示のエラー件数、キーボード操作、Cmd/Ctrl+F で検索できるイベントトレース詳細（`t=` / `st=` / `[dt=]`、一致移動・大文字小文字区別・正規表現トグル）に対応。HTTP リクエストを選択すると Request/Response サマリー（メソッド・URL・ステータス・ヘッダー・所要時間）が表示されます。
  - **Timeline** — ソケット数・リクエスト数・転送バイト数などの時系列をキャンバスに描画。マウスのほかキーボード（矢印キー/+-/Home）でもズーム・パンでき、ホバーで値を確認でき、現在のズーム倍率と表示中の時間帯も常時表示されます。グラフをダブルクリックすると、その時点でアクティブだったソースに絞り込んだ状態で Events タブへジャンプします。
  - **Proxy / DNS / Sockets / StreamPool / Alt-Svc / HTTP/2 / QUIC / Reporting / Cache / Modules / Prerender** — 各サブシステムの状態テーブル。フリーテキストの行フィルタと列の並び替えに対応。

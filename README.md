...
- **14-tab layout**:
  ...
  - **Events** — A list of events grouped by source, with filtering (`type:` / `id:` / `is:active|error` / `sort:` / `has:<text>` for cross-source parameter search, also via sortable column headers) and in-app syntax help, an always-visible error count, keyboard navigation, and event trace details (`t=` / `st=` / `[dt=]`). Selecting an HTTP request also shows a Request/Response summary (method, URL, status, headers, timing).

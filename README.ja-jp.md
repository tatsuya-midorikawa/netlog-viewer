...
- **大容量ログに対応**: ファイルを解析しながらディスクからストリーミング読み込みするため、数百 MB〜数 GB のログでも約 512 MB の文字列サイズ上限に達することなく開けます。読み込みの進捗や切り詰めの警告は画面内のバナーに表示します。
...
| 設定 | 既定値 | 説明 |
| --- | --- | --- |
| `netlogViewer.maxEvents` | `2000000` | 1 ファイルから読み込むイベントの最大数。これを超えるファイルはディスクからストリーミング読み込みし、この件数で切り詰めます（警告付き）。メモリ使用量を抑えるための上限です。`0` を指定するとすべてのイベントを読み込みます（非常に大きなログではメモリを使い切る場合があります）。 |
| `netlogViewer.maxSourceDetailEvents` | `50000` | 選択した 1 つのソースについて詳細ペインに表示するイベントの最大数。これを超えるソースは切り詰めて表示します（注記付き）。非常に活発なソースを選んでも応答性を保つための上限です。 |
...
- Events タブのフリーテキスト フィルターは、ソースの id・種類・説明を対象に検索します（各イベントのパラメーター内は検索しません）。選択したソースの詳細は必要に応じて読み込みます。
- Events のソース一覧は一度に最大 2,000 件までを表示します（すべてのソースはフィルター対象のままなので、フィルターで絞り込むと残りも確認できます）。
